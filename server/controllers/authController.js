...
export const sendOtpEmail = async (email, otp) => {
  try {
    ...
    const mailOptions = {
      ...
      html: `<p>Your OTP is:</p><h2>${otp}</h2><p>This OTP expires in 5 minutes.</p>`,
    };
  ...
  static sendLogoutEmail = async (req, res) => {
    ...
    try {
      ...
      return res.status(200).json({
        success: true,
        message: "Logout email sent",
      });
    } catch (error) {
      console.error("❌ Logout Email Error:", error);
      return res.status(500).json({
        success: false,
        message: "Failed to send logout email",
      });
    }
  };
  ...
  static async sendLogoutEmail(req, res) {
    ...
    try {
      ...
      const transporter = nodemailer.createTransport({
        ...
        auth: {
          type: "OAuth2",
          user: process.env.OAUTH_USER_EMAIL, // Gmail email
          clientId: process.env.OAUTH_CLIENT_ID,
          clientSecret: process.env.OAUTH_CLIENT_SECRET,
          refreshToken: process.env.OAUTH_REFRESH_TOKEN,
          accessToken: accessToken.token,
      ...
      // -------------------- Send Email --------------------
      await transporter.sendMail(mailOptions);
      console.log("📩 Logout email sent:", email);
  ...
  static checkSession = async (req, res, next) => {
    try {
      ...
      if (inactiveTime > 5 * 60 * 1000) {
        ...
        return res
          .status(401)
          .json({ message: "Auto logged out due to inactivity" });
      }
 ...
 */
 ...
 static forgetPassword = async (req, res) => {
    ...
    try {
      ...
      // -----------------------------------------
      // EMAIL CONTENT (Same HTML You Provided)
      // -----------------------------------------
      const mailOptions = {
        from: process.env.OAUTH_USER_EMAIL,
        to: email,
...
<a href="${link}"
   style="background:#20e277; color:#fff; padding:10px 24px; margin-top:35px;
